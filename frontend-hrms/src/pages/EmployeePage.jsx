...
const EmployeePage = () => {
  ...
  const fetchEmployees = async (currentPage = page, limit = rowsPerPage) => {
    try {

      const res = await api.get(`/employees?page=${currentPage + 1}&limit=${limit}`);

      const data = res.data;

      setEmployees(data.employees || []);
      setTotalCount(data.pagination?.totalEmployees || 0);
